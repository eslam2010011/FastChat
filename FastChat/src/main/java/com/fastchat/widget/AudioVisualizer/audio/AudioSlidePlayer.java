...
import static com.google.android.gms.common.util.zzc.isMainThread;
...
public class AudioSlidePlayer {
  ...
  public void requestDuration() {
    try {
      ...
      durationCalculator.prepare(createMediaSource(Uri.parse(slide.getAttachment().getFileUrl())));
    } catch (Exception e) {
        Log.w(TAG, e);
  ...
  private void play(final double progress, boolean earpiece) throws IOException {
    ...
    if (slide.getAttachment().getFileUrl() == null) {
    ...
    mediaPlayer.prepare(createMediaSource(Uri.parse(slide.getAttachment().getFileUrl())));
  ...
  public @NonNull MediaMessage getAudioSlide() {
    return slide;
  }
